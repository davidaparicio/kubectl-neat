...
jq 'delpaths([path(.spec.platforms[] | select( .selector.matchLabels.os != $os or .selector.matchLabels.arch != $arch ))])' --arg os "$os" --arg arch "$arch" "$tmp" > "$tmp.new" && mv "$tmp.new" "$tmp"
jq '.metadata.name = $name' --arg name "$plugin" "$tmp" > "$tmp.new" && mv "$tmp.new" "$tmp"
jq 'setpath(path(.spec.platforms[] | select( .selector.matchLabels.os == $os and .selector.matchLabels.arch == $arch) | .sha256); $sha)' --arg os "$os" --arg arch "$arch" --arg sha "$sha256" "$tmp" > "$tmp.new" && mv "$tmp.new" "$tmp"
